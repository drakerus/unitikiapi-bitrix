...
define('MYSQL_USER', 'ph382841_root');
define('MYSQL_PASSWORD', 'Gu5xKeKPK4EE');
define('MYSQL_DATABASE', 'ph382841_mybuses');
...
define('RESULT_DIR', '/var/www/ph382841/data/www/ph382841-160620151442031.www12.pagehost.ru/mods/bus/town');
...
foreach ($town_ids as $id_town) {
	...
	foreach (array($bus_f_dir, $bus_b_dir) as $race_direction => $bus_dir) {
		...
		foreach ($buses_by_id as $bus) {
			...
			file_put_contents("{$bus_dir}/{$bus->alias}_sh_selector.html", ob_get_clean());

			ob_start();
			require PROTOTYPE_DIR."/bus/bus_alias.php";
			file_put_contents("{$bus_dir}/{$bus->alias}.html", ob_get_clean());
		}
	}
	
	unset($times, $stations_ids, $rows, $stations);
	...
	foreach (array($station_f_dir, $station_b_dir) as $race_direction => $station_dir) {
		foreach ($stations_by_id as $station) {
			...
			file_put_contents("{$station_dir}/{$station->alias}_sh_legend.html", ob_get_clean());

			ob_start();
			require PROTOTYPE_DIR."/station/station_alias_sh_selector.php";
			file_put_contents("{$station_dir}/{$station->alias}_sh_selector.html", ob_get_clean());

			ob_start();
			require PROTOTYPE_DIR."/station/station_alias.php";
			file_put_contents("{$station_dir}/{$station->alias}.html", ob_get_clean());
