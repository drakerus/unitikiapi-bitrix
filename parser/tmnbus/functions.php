...
function request($url, $referer = 'http://tgt72.ru/schedule/')
{
    $options = array(
        CURLOPT_HEADER => true,
...
function request_multi($urls, $referer = 'http://tgt72.ru/schedule/')
{
    $options = array(
        CURLOPT_HEADER => true,
        CURLOPT_RETURNTRANSFER => true,
    ...
    foreach (array_chunk($urls, 50, true) as $chunk)
    {
        $errors = array();
        $u = $chunk;
