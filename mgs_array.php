...
$_SERVER["DOCUMENT_ROOT"]='/var/www/ph382841/data/www/mybuses.ru';
    ...
    try{
        ...
        // устанавливаем тип парсера - атобусы, троллейбусы, трамваи
    ...
    /** Класс расширяет класс Exception,
     *  чтобы при выбросе исключения было понятно, что проблема в парсере
     * */
    class MGSException extends Exception{
        /**
         * Конструктор - принимает сообщение об ошибке и код.
         * Инициализирует родительский класс Exception с добавлением своей инофрмации.
         * */
    ...
    class MGSParser{
        ...
        # массив маршрутов; формат: 'Номер автобуса' => array('AB' => 'название прямого направления',
        #                                                     'BA' => 'название обратного направления')
        ...
        # массив типов парсера
        private $_types = array('avto', 'trol', 'tram');
        private $_type = '';
        
        private $_raw_timetable = '';
        
        # массивы для преобразования "дней" из формата донора в человекочитаемый вид
        ...
        # массив преобразования "дней" из формата донора в формат битрикса
        ...
        /**
         * Конструктор класса. 
         * $town_id - id Города в битриксе
         * $timetable_id - id инфоблока с расписанием
         * $stations_id - id инфоблока с остановками
         * $buses_id - id инфоблока с маршрутами
         * */
        ...
        /**
         * Проверяет и устанавливает тип парсера
         * string $type - тип парсера. Добустимые типы перечислены в массиве MGSParser::$_types
         * */
        ...
        /**
         * Основной метод - собирает и сохраняет данные
         * 
         * */
        public function collectData(){
            ...
            foreach($routes as $route){
                if( trim( preg_replace('#[^а-я0-9]#','', $route)) == '') continue;
                $bus = array();
                #$this->_dropBus($route);
                ...
                foreach($days as $day){
                    ...
                    $this->_routes[$route] = $real_directions;
                    foreach($real_directions as $direction => $value){
                        ...
                        if( false !== $timetable = $this->_parseTimetable()){
                            $bus[$day][$direction] = $this->_formatArray($timetable);
                ...
                //file_put_contents(SAVE_PATH.mb_convert_encoding($route, 'utf8', 'cp1251').'.data', serialize($bus));
				
                file_put_contents(SAVE_PATH.$this->_transletiration(mb_convert_encoding($route, 'utf8', 'cp1251')).'.data', serialize($bus));
            }
            
            file_put_contents(SAVE_PATH.'routes.data', serialize($this->_routes));
            file_put_contents(SAVE_PATH.'stations.data', serialize($this->_stations));
        }
        
        
        /**
         * Форматирует массив с расписанием для запсиси в файл
         * */
        private function _formatArray($timetable){
            $schedule = array();
            foreach($timetable as $station => $races){
                $schedule[$station] = array();
                foreach($races as $race_index => $race_time){
                   $schedule[$station][$race_index+1] = $race_time; 
                }
            }
            return $schedule;
        }
        ...
        /**
         * Сохраняет расписание с использованием API битрикса
         * */
        ...
        /**
         * Сохраняет маршрут с использованием API битрикса
         * */
        ...
        /**
         * Обновляет маршрут с использованием API битрикса - добавляются названия направлений
         * */
        ...
        /**
         * Сохраняет остановку с использованием API битрикса
         * */
        ...
        /**
         * Метод производит парсинг страницы донора, которая содержит расписание
         * */
        ...
        /**
         * Очистка инфоблоков битрикса перед началом парсинга
         * */
        ...
        /**
        * Преобразует "дни" из формата донора в человекочитаемый
        */
        ...
        /**
        * Получает страницу донора с использованием curl.
        * Производит анализ http-кода ответа.
        * */
        ...
        /**
         * Очищает строку от лишних пробелов и переводов строк для упрощения парсинга
         * */
