...
?>
<script>$(function(){function displayResultBus(item){if(item.text!='Result not Found'){var prefix='http://mybuses.ru/';var postfix='/bus/';var postfixxx='/';var town='<?=$town;?>';var src=prefix+town+postfix+item.value+postfixxx;window.open(src, "_self");}else{$('.alert-bus').show().html('Увы ничего не найдено, попытайтесь снова');}}$('#bus_typeahead').typeahead({source:[<? foreach($super_bus_array as $key=>$value){?>{id:'<?=$key;?>', name:'<?=$value;?>'},<?}?>],onSelect: displayResultBus});});</script>
<?
