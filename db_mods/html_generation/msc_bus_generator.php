...
foreach($all_buses_array as $bus_id=>$bus_prop_array){
	...
	//Бежим по 2 направлениям - вперед/назад
		...
		$sql="SELECT * FROM `time` WHERE id_town='".TOWN_ID."' AND id_bus='".$bus_id."' AND race_direction='".$direction_id."' ;";
	...
	*/
		...
		if($ykey==-1){?><tr><?}
			...
			foreach($x_coord as $xkey=>$xvalue){
				...
				if(!empty($super_array[$xvalue][$ykey]['TIME'])){ echo $super_array[$xvalue][$ykey]['TIME']; }	//Выводим время			
		...
		if(!is_dir($dir4)){ mkdir($dir4); chmod($dir4, 777); }
		/*
