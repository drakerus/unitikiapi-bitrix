...
class CacheClass{
    ...
    public function generateCache(){
        ...
        exec('chown -R admin.admin '.CACHE_DIR.'*');
    ...
    private function _processWay($all_threads, $thread_name, $thread_alias, $way_dir, $town_id, $shift = 0)
    {
        ...
        $thread['thread_alias'] = $thread_alias; // for BBC
        $this->_generateFile(
            self::$way_template,
            array(
                'info' => $thread,
        ...
        foreach($schedules as $k => $schedule){
            ...
            for( $day = strtotime( $schedules[$k]['shedule_start_date'] ); $day <= strtotime( $schedules[$k]['shedule_end_date'] ); $day = strtotime('+1 day', $day)  ){
                if( in_array( date('N', $day), $days ) ){
                    ...
                    $schedules[$k]['dates'][$start_time]['start_month_short'] = $this->_translateMonth( date('n', strtotime($start_time)), true );
                    $schedules[$k]['dates'][$start_time]['stop_month'] = $this->_translateMonth( date('n', strtotime($schedules[$k]['dates'][$start_time]['stop_time'])) );
                    $schedules[$k]['dates'][$start_time]['stop_month_short'] = $this->_translateMonth( date('n', strtotime($schedules[$k]['dates'][$start_time]['stop_time'])), true );
    ...
    private function _translateMonth( $month_num, $short = false ){
        $months = array(
            1 => 'января',
            2 => 'февраля',
            3 => 'марта',
        ...
        return (  ( true === $short ) ? @$short_months[intval($month_num)] : @$months[intval($month_num)] );
    }
