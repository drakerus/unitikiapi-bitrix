...
class CacheClass{
    ...
    private function _processWay( $all_threads, $thread_name, $thread_alias, $way_dir, $town_id, $shift = 0 ){
        foreach($all_threads as $k => $thread){
            ...
            $all_threads[$k]['schedules'] = $this->_getSchedules( 
                                            $thread['id_thread'], 
                                            $thread['to_a_shift'],
                                            array(
                                                'date' => '',
        ...
        foreach($all_threads as $k => $tt) {
            ...
            foreach ($all_threads[$k]['schedules'] as $schedule) {
                foreach ($schedule as $date => $data) {
                    ...
                    $data['travel_time'] = $tt['travel_time'];
                    $data['from_station'] = $tt['from_station'];
                    $data['to_station'] = $tt['to_station'];
                    $data['prices'] = $tt['prices'];
                    $data['thread_name'] = $tt['thread_name'];
                    $schedules[] = $data;
                }
            }
        }
        ...
        $this->_generateFile(self::$way_template, array('info' => $tt, 'schedules' => $schedules, 'stations' => self::_prepareStations($stations)), $way_dir . $thread_alias . '.html');
