...
/*
		...
		$PROP = array();
		$PROP[205] = $town_id;//Город
		$PROP[206] = $bus_type;//Тип
		$PROP[220] = $alias;//АЛИАС
		$PROP[208] = $info['way'][$j];//Направление
			if(!empty($id_subway)){
			$PROP[214] = $id_subway;//Метро
			}
		$PROP[219] = $info['src'][$j];//Ссылка		
		...
		$arLoadProductArray = Array(
			  ...
			  "NAME"           => $info['bus_nuber'][$j],
			  "ACTIVE"         => "Y",            // активен			  
			  );

			 
		if($PRODUCT_ID = $el->Add($arLoadProductArray)){  echo "<br/>Добавлен автоубс №: ".$info['bus_nuber'][$j]; }
...
$target_href='http://podolsk-now.ru/mods/bus/new/parse/station_cheker.php';
?><!--
