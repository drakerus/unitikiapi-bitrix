...
session_start();
if(($_SESSION["parse"]['step_3']!='OK')OR(empty($_SESSION["parse"]['town_alias']))OR(empty($_SESSION["parse"]['id_town']))){  $_SESSION["fail_step"]='step_3'; }
	...
	//$key_tr - номер строки		
		...
		if(!preg_match('/[0-9]/', $time)){$time='#';}
				...
				if(!in_array($unq,$duplication_array)OR($time=='#')){
					...
					echo '<pre>'; print_r($stations_db_array);
					
					$_SESSION['parse']['error_desc'][]=$shedule.' sql: '.$sql.'<br/> SHEDULE: '.$race_type_arr[$key][$race].'<br/> STATION: '.$stops_name_arr[$key][$key_tr];
...
if($num>=$mnum){	
$_SESSION["parse"]["step_4"]="OK";
$_SESSION["parse"]["current_step"]="step_4";
echo 'OK';
