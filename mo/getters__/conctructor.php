...
define('SAVE_PATH', '/home/bitrix/www/mods/bus/mo/log/');
?>
<?
...
if($_SESSION['parse']["current_step"]=="step_8"){//После последнего успешно выполненного скрипта меняем город	
	...
	if($new_id_town<=end($town_array)){
		
			 
		$sql='UPDATE  `mybuses`.`cursor` SET  `id_town` =  '.$new_id_town.' WHERE  `cursor`.`id_cursor` =1;';		
		//echo $sql; die();
		mysql_query($sql);
		...
		?><script language="javascript">window.location.href = '<?=$target_href[10];?>';</script><?	
	}
	else{
	die('constructor - OK');	
	}
